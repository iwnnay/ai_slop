...
async function generateImportantEvent(date) {
    const prompt = `What were the most important global events that happened on ${date}? Please list 3-5 major events that occurred on this date in 2014. Include significant news, sports, cultural, or historical events.`;
...
function saveEvents(date, events) {
    const filename = `${date.replace(/-/g, '')}.txt`;
